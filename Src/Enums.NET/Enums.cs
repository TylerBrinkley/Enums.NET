...
namespace EnumsNET
{
    ...
    internal static class Enums<TEnum>
    {
        ...
        static Enums()
        {
...
#if NET20 || USE_EMIT
            var toInt = new DynamicMethod(type.Name + "_ToInt",
                                       underlyingType,
                                       new[] { type },
                                       type, true);
            ...
            var toIntDelegate = toInt.CreateDelegate(typeof(Func<,>).MakeGenericType(type, underlyingType));
            ...
            toEnumGenerator.DeclareLocal(type);
            ...
            var toEnumDelegate = toEnum.CreateDelegate(typeof(Func<,>).MakeGenericType(underlyingType, type));
#else
            ...
            {
                case TypeCode.Int32:
                    Cache = new EnumsCache<TEnum, int>(
#if NET20 || USE_EMIT
                        (Func<TEnum, int>)toIntDelegate,
                        (Func<int, TEnum>)toEnumDelegate
#else
                        Expression.Lambda<Func<TEnum, int>>(enumParamConvert, enumParam).Compile(),
                        Expression.Lambda<Func<int, TEnum>>(intParamConvert, intParam).Compile()
#endif
                        );
                    break;
                case TypeCode.UInt32:
                    Cache = new EnumsCache<TEnum, uint>(
#if NET20 || USE_EMIT
                        (Func<TEnum, uint>)toIntDelegate,
                        (Func<uint, TEnum>)toEnumDelegate
#else
                        Expression.Lambda<Func<TEnum, uint>>(enumParamConvert, enumParam).Compile(),
                        Expression.Lambda<Func<uint, TEnum>>(intParamConvert, intParam).Compile()
#endif
                        );
                    break;
                case TypeCode.Int64:
                    Cache = new EnumsCache<TEnum, long>(
#if NET20 || USE_EMIT
                        (Func<TEnum, long>)toIntDelegate,
                        (Func<long, TEnum>)toEnumDelegate
#else
                        Expression.Lambda<Func<TEnum, long>>(enumParamConvert, enumParam).Compile(),
                        Expression.Lambda<Func<long, TEnum>>(intParamConvert, intParam).Compile()
#endif
                        );
                    break;
                case TypeCode.UInt64:
                    Cache = new EnumsCache<TEnum, ulong>(
#if NET20 || USE_EMIT
                        (Func<TEnum, ulong>)toIntDelegate,
                        (Func<ulong, TEnum>)toEnumDelegate
#else
                        Expression.Lambda<Func<TEnum, ulong>>(enumParamConvert, enumParam).Compile(),
                        Expression.Lambda<Func<ulong, TEnum>>(intParamConvert, intParam).Compile()
#endif
                        );
                    break;
                case TypeCode.SByte:
                    Cache = new EnumsCache<TEnum, sbyte>(
#if NET20 || USE_EMIT
                        (Func<TEnum, sbyte>)toIntDelegate,
                        (Func<sbyte, TEnum>)toEnumDelegate
#else
                        Expression.Lambda<Func<TEnum, sbyte>>(enumParamConvert, enumParam).Compile(),
                        Expression.Lambda<Func<sbyte, TEnum>>(intParamConvert, intParam).Compile()
#endif
                        );
                    break;
                case TypeCode.Byte:
                    Cache = new EnumsCache<TEnum, byte>(
#if NET20 || USE_EMIT
                        (Func<TEnum, byte>)toIntDelegate,
                        (Func<byte, TEnum>)toEnumDelegate
#else
                        Expression.Lambda<Func<TEnum, byte>>(enumParamConvert, enumParam).Compile(),
                        Expression.Lambda<Func<byte, TEnum>>(intParamConvert, intParam).Compile()
#endif
                        );
                    break;
                case TypeCode.Int16:
                    Cache = new EnumsCache<TEnum, short>(
#if NET20 || USE_EMIT
                        (Func<TEnum, short>)toIntDelegate,
                        (Func<short, TEnum>)toEnumDelegate
#else
                        Expression.Lambda<Func<TEnum, short>>(enumParamConvert, enumParam).Compile(),
                        Expression.Lambda<Func<short, TEnum>>(intParamConvert, intParam).Compile()
#endif
                        );
                    break;
                case TypeCode.UInt16:
                    Cache = new EnumsCache<TEnum, ushort>(
#if NET20 || USE_EMIT
                        (Func<TEnum, ushort>)toIntDelegate,
                        (Func<ushort, TEnum>)toEnumDelegate
#else
                        Expression.Lambda<Func<TEnum, ushort>>(enumParamConvert, enumParam).Compile(),
