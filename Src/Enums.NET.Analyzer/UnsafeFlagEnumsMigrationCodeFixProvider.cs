...
{
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(UnsafeFlagEnumsMigrationCodeFixProvider)), Shared]
    public class UnsafeFlagEnumsMigrationCodeFixProvider : CodeFixProvider
    {
        private const string title = "Migrate UnsafeFlagEnums to Enums";

        public sealed override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create(UnsafeFlagEnumsMigrationAnalyzer.DiagnosticId);
        ...
        private async Task<Document> MigrateToEnumsAsync(Document document, IdentifierNameSyntax memberAccessNode, CancellationToken cancellationToken)
        {
            ...
            newRoot = newRoot.ReplaceNode(newRoot.FindNode(memberAccessNode.Span), SyntaxFactory.IdentifierName("FlagEnums"));
