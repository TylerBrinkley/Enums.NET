properties {
  ...
  $builds = @(
    ...
    @{Framework = "netstandard1.3"; TestsFunction = "NUnitTests"; TestFramework = "netcoreapp1.1"; NUnitFramework = "netcoreapp1.1"; Enabled = $true},
    @{Framework = "netstandard1.1"; TestsFunction = "NUnitTests"; TestFramework = "netcoreapp1.1"; NUnitFramework = "netcoreapp1.1"; Enabled = $true},
    @{Framework = "netstandard1.0"; TestsFunction = "NUnitTests"; TestFramework = "netcoreapp1.0"; NUnitFramework = "netcoreapp1.0"; Enabled = $true}
  )
}
...
task Build -depends Clean {
  ...
  $xml = [xml](Get-Content "$workingSourceDir\Enums.NET\Enums.NET.csproj")
  ...
  $xml.save("$workingSourceDir\Enums.NET\Enums.NET.csproj")

  $projectPath = "$workingSourceDir\Enums.NET\Enums.NET.csproj"
...
task Package -depends Build {
  foreach ($build in $script:enabledBuilds)
  {
    ...
    $sourcePath = "$workingSourceDir\Enums.NET\bin\Release\$finalDir"
  ...
  if ($buildNuGet)
  {
    ...
    exec { & $script:msBuildPath "/t:pack" "/p:IncludeSource=true" "/p:Configuration=Release" "/p:TargetFrameworks=`"$targetFrameworks`"" "$workingSourceDir\Enums.NET\Enums.NET.csproj" }

    mkdir $workingDir\NuGet
    move -Path $workingSourceDir\Enums.NET\bin\Release\*.nupkg -Destination $workingDir\NuGet
  }
...
{
  $projectPath = "$workingSourceDir\Enums.NET.sln"
...
{
  $projectPath = "$workingSourceDir\Enums.NET.Tests\Enums.NET.Tests.csproj"
  $location = "$workingSourceDir\Enums.NET.Tests"
...
function NUnitTests($build)
{
  ...
  Write-Host
  robocopy "$workingSourceDir\Enums.NET.Tests\bin\Release\$testDir" $testRunDir /MIR /NFL /NDL /NJS /NC /NS /NP /XO | Out-Default
  ...
  try
  {
    ...
    exec { & $nunitConsolePath\tools\nunit3-console.exe "$testRunDir\Enums.NET.Tests.dll" --framework=$framework --result=$workingDir\$testDir.xml | Out-Default } "Error running $testDir tests"
  }
  finally
  {
    Set-Location $baseDir
